...
import { createSignup, fetchSignups, unregisterSignup } from './features/signups/api'
...
import { fetchActiveLocation, setActiveLocation } from './features/settings/api'
    ...
    <div className="min-h-dvh px-4 pb-[calc(env(safe-area-inset-bottom)+2.5rem)] pt-6 sm:px-6">
      <div className="mx-auto w-full max-w-md">
        ...
        <main className="mt-6 space-y-4">
          <section className="rounded-2xl border border-[--border] bg-[--surface] p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <button
                  type="button"
                  className="text-left text-sm font-semibold focus:outline-none focus-visible:ring-2 focus-visible:ring-[--gold] rounded-md"
                  onPointerDown={() => {
                    if (!supabase) return
          ...
          {supabase && mySignup && myDeleteToken ? (
            <button
              type="button"
              className="w-full rounded-2xl border border-[--border] bg-black/20 px-4 py-3 text-sm font-semibold text-white/90 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60"
              disabled={submitting}
              onClick={async () => {
                setSubmitting(true)
                setError(null)
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const data = await fetchSignups({ playDate, location: l.id })
                      setSignups(data)
                      setAdminOpen(false)
                    } catch {
