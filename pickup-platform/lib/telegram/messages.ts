...
export function formatRsvpReply(opts: {
	...
	const head = headcount != null ? ` · ${headcount} confirmed` : "";
...
export function formatPaidSessionMessage(url: string): string {
	return ["This session requires payment — sign up on the web:", url].join(
		"\n",
	);
}
...
export function formatMvpAnnouncement(opts: {
	...
	return [`🏆 ${orgName} — ${sessionTitle}`, when, winners, eventUrl].join(
		"\n",
	);
}

export function formatGroupLinkedMessage(
	orgName: string,
	orgSlug: string,
): string {
	return [
