-- Idempotent (re-)apply of notification dismissals from 029.
-- Run this if clear/dismiss RPCs are missing from the schema cache.

create table if not exists public.organizer_notification_dismissals (
...
drop policy if exists "Members can view own dismissals"
  on public.organizer_notification_dismissals;
create policy "Members can view own dismissals"
  on public.organizer_notification_dismissals for select
  to authenticated
  using (user_id = auth.uid());

drop policy if exists "Members can dismiss notifications"
  on public.organizer_notification_dismissals;
