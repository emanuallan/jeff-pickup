-- Headcount Phase 4: caps, streaks, engagement stats (per org)

-- ---------------------------------------------------------------------------
-- Extend public roster view with participant_id (needed for badge lookups)
-- Postgres cannot reorder view columns via CREATE OR REPLACE — drop first.
-- ---------------------------------------------------------------------------

drop view if exists public.event_roster_public;

create view public.event_roster_public as
select
  s.id,
  s.event_id,
  s.org_id,
...
-- ---------------------------------------------------------------------------
-- Caps leaderboard: distinct past sessions per participant
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Weekly streak leaderboard (current streak ending in week of p_as_of)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Per-participant stats for roster badges
-- ---------------------------------------------------------------------------
...
grant execute on function public.org_caps_leaderboard(uuid, timestamptz, int) to anon, authenticated;
grant execute on function public.org_weekly_streak_leaderboard(uuid, date, int) to anon, authenticated;
grant execute on function public.participant_engagement_stats(uuid, uuid[], date) to anon, authenticated;
