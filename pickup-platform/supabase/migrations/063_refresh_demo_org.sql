-- Refresh demo org (slug: demo) — schema-current through 062.
...
--
-- Showcases: live roster, leaderboard, session feed (MVP + stats + reactions),
-- session feedback, and group rules.
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'group_rules', jsonb_build_object(
        'version', 1,
        'text', E'Welcome to Music City Pickup!\n\n• Play fair and keep the vibe friendly.\n• Respect the ref and other players.\n• No slide tackles on turf.\n• Help set up goals if you arrive early.\n• If you RSVP, try to show up — or update your status.'
      )
    ),
  ...
  values
    ...
    (p_nick, v_org_id, '12025550115', 'Nick', 'Foster', 'Nick'),
    (p_sara, v_org_id, '12025550116', 'Sara', 'Mitchell', 'Sara'),
    (p_omar, v_org_id, '12025550117', 'Omar', 'Farouk', 'Omar'),
    (p_kate, v_org_id, '12025550118', 'Kate', 'Lindsey', 'Kate'),
    (p_luis, v_org_id, '12025550119', 'Luis', 'Morales', 'Luis'),
    (p_hannah, v_org_id, '12025550120', 'Hannah', 'Wright', 'Hannah'),
    (p_james, v_org_id, '12025550121', 'James', 'Porter', 'James'),
    (p_aisha, v_org_id, '12025550122', 'Aisha', 'Khan', 'Aisha'),
    (p_connor, v_org_id, '12025550123', 'Connor', 'Hayes', 'Connor'),
    (p_zoe, v_org_id, '12025550124', 'Zoe', 'Barnes', 'Zoe'),
    (p_miguel, v_org_id, '12025550125', 'Miguel', 'Torres', 'Miguel'),
    (p_lily, v_org_id, '12025550126', 'Lily', 'Nguyen', 'Lily'),
    (p_alex, v_org_id, '12025550127', 'Alex', 'Rivera', 'Alex'),
    (p_nora, v_org_id, '12025550128', 'Nora', 'Bell', 'Nora');
  ...
  -- Session feed: MVP results, player stats, and reactions on the most recent pickup
  insert into public.session_mvp_votes (org_id, event_id, voter_participant_id, nominee_participant_id)
  values
    (v_org_id, v_ev_past1, p_marcus, p_diego),
    (v_org_id, v_ev_past1, p_diego, p_jen),
    (v_org_id, v_ev_past1, p_jen, p_marcus),
    (v_org_id, v_ev_past1, p_tyler, p_marcus),
    (v_org_id, v_ev_past1, p_sofia, p_diego),
    (v_org_id, v_ev_past1, p_chris, p_jen),
    (v_org_id, v_ev_past1, p_amir, p_marcus),
    (v_org_id, v_ev_past1, p_elena, p_diego),
    (v_org_id, v_ev_past1, p_jordan, p_jen);

  insert into public.session_mvp_finalizations (event_id, org_id, finalized_at, total_votes)
  values (v_ev_past1, v_org_id, now() - interval '10 hours', 9);

  insert into public.session_mvp_awards (
    org_id, event_id, participant_id, vote_count, finalized_at, badge_expires_at
  )
  values
    (
      v_org_id, v_ev_past1, p_marcus, 3, now() - interval '10 hours',
      (v_ev_next_day + time '20:00') at time zone v_tz
    ),
    (
      v_org_id, v_ev_past1, p_diego, 3, now() - interval '10 hours',
      (v_ev_next_day + time '20:00') at time zone v_tz
    ),
    (
      v_org_id, v_ev_past1, p_jen, 3, now() - interval '10 hours',
      (v_ev_next_day + time '20:00') at time zone v_tz
    );

  -- Second pickup: clear MVP winner with runners-up
  insert into public.session_mvp_votes (org_id, event_id, voter_participant_id, nominee_participant_id)
  values
    (v_org_id, v_ev_past2, p_marcus, p_tyler),
    (v_org_id, v_ev_past2, p_diego, p_tyler),
    (v_org_id, v_ev_past2, p_jen, p_tyler),
    (v_org_id, v_ev_past2, p_tyler, p_maya),
    (v_org_id, v_ev_past2, p_sofia, p_tyler),
    (v_org_id, v_ev_past2, p_chris, p_tyler),
    (v_org_id, v_ev_past2, p_amir, p_maya),
    (v_org_id, v_ev_past2, p_elena, p_tyler),
    (v_org_id, v_ev_past2, p_jordan, p_leo),
    (v_org_id, v_ev_past2, p_maya, p_tyler),
    (v_org_id, v_ev_past2, p_leo, p_maya);

  insert into public.session_mvp_finalizations (event_id, org_id, finalized_at, total_votes)
  values (v_ev_past2, v_org_id, now() - interval '1 day 8 hours', 11);

  insert into public.session_mvp_awards (org_id, event_id, participant_id, vote_count, finalized_at, badge_expires_at)
  values (
    v_org_id, v_ev_past2, p_tyler, 7, now() - interval '1 day 8 hours',
    (v_ev_next_day + time '20:00') at time zone v_tz
  );

  insert into public.session_player_stats (org_id, event_id, participant_id, goals, assists, created_at)
  values
    (v_org_id, v_ev_past1, p_luis, 2, 1, now() - interval '14 hours'),
    (v_org_id, v_ev_past1, p_marcus, 1, 0, now() - interval '15 hours'),
    (v_org_id, v_ev_past1, p_diego, 1, 2, now() - interval '15 hours'),
    (v_org_id, v_ev_past1, p_jen, 0, 2, now() - interval '16 hours'),
    (v_org_id, v_ev_past1, p_tyler, 1, 0, now() - interval '16 hours'),
    (v_org_id, v_ev_past2, p_maya, 3, 0, now() - interval '1 day 9 hours'),
    (v_org_id, v_ev_past2, p_leo, 1, 2, now() - interval '1 day 9 hours');

  insert into public.org_session_feed_reactions (
    org_id, feed_kind, event_id, subject_participant_id, reactor_participant_id, emoji, created_at
  )
  values
    (v_org_id, 'mvp', v_ev_past1, null, p_tyler, '🐐', now() - interval '9 hours'),
    (v_org_id, 'mvp', v_ev_past1, null, p_sofia, '🔥', now() - interval '8 hours'),
    (v_org_id, 'mvp', v_ev_past1, null, p_chris, '👏', now() - interval '7 hours'),
    (v_org_id, 'mvp', v_ev_past1, null, p_amir, '👀', now() - interval '6 hours'),
    (v_org_id, 'player_stats', v_ev_past1, p_luis, p_marcus, '🔥', now() - interval '13 hours'),
    (v_org_id, 'player_stats', v_ev_past1, p_luis, p_jen, '👏', now() - interval '12 hours'),
    (v_org_id, 'player_stats', v_ev_past1, p_diego, p_elena, '⚽', now() - interval '14 hours'),
    (v_org_id, 'player_stats', v_ev_past2, p_maya, p_tyler, '🐐', now() - interval '1 day 7 hours');

  -- Session feedback for console + debrief history
  insert into public.session_feedback (org_id, event_id, participant_id, outcome, rating, comment, created_at)
  values
    (v_org_id, v_ev_past1, p_marcus, 'rated', 5, null, now() - interval '11 hours'),
    (v_org_id, v_ev_past1, p_diego, 'rated', 5, 'Great turnout and good energy.', now() - interval '11 hours'),
    (v_org_id, v_ev_past1, p_jen, 'rated', 4, null, now() - interval '10 hours'),
    (v_org_id, v_ev_past1, p_tyler, 'rated', 5, 'Loved the small-sided games at the end.', now() - interval '10 hours'),
    (v_org_id, v_ev_past1, p_sofia, 'rated', 4, null, now() - interval '9 hours'),
    (v_org_id, v_ev_past1, p_chris, 'no_attend', null, null, now() - interval '9 hours'),
    (v_org_id, v_ev_past2, p_marcus, 'rated', 4, null, now() - interval '1 day 7 hours'),
    (v_org_id, v_ev_past2, p_diego, 'rated', 5, 'Tyler was on fire this week.', now() - interval '1 day 7 hours'),
    (v_org_id, v_ev_past2, p_maya, 'rated', 5, null, now() - interval '1 day 6 hours');
end;
$$;
