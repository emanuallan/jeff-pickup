-- schedules.price_cents — same semantics as events.price_cents (null/0 = free).
-- Newly materialized sessions inherit the schedule's session fee.

alter table public.schedules
  add column if not exists price_cents int
  check (price_cents is null or price_cents >= 0);

comment on column public.schedules.price_cents is
  'Nullable session fee in cents inherited by newly materialized events. Null or 0 = free.';
          ...
          if v_dow = any(r.byweekday) then
            ...
            if not exists (
              ...
              insert into public.events (
                ...
                additional_information, team_count, price_cents
              )
              values (
                ...
                r.timezone, r.additional_information, r.team_count, r.price_cents
              )
