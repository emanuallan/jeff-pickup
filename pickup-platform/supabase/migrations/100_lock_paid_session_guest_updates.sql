-- Lock participant guest-count changes after signup on paid sessions.
-- Checkout charges per person, so post-pay guest edits would desync headcount and payment.
...
begin
  ...
  select e.price_cents into v_price_cents from public.events e where e.id = v_event_id;
  if v_price_cents is not null and v_price_cents > 0 then
    raise exception 'Guest count cannot be changed after you pay for this session.';
  end if;
