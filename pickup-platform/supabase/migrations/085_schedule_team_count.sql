-- schedules.team_count — same semantics as events.team_count (null = off, 2–8 when on).
-- Newly materialized sessions inherit the schedule's team_count.

alter table public.schedules
  add column if not exists team_count int
  check (team_count is null or (team_count >= 2 and team_count <= 8));

comment on column public.schedules.team_count is
  'When set (2–8), materialized sessions inherit this team_count. Null = no teams.';
          ...
          if v_dow = any(r.byweekday) then
            ...
            if not exists (
              ...
              insert into public.events (
                ...
                additional_information, team_count
              )
              values (
                ...
                r.timezone, r.additional_information, r.team_count
              )
