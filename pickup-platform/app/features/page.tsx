...
export const metadata: Metadata = buildMarketingPageMetadata(
  '/features',
  'Features',
  'Organizr helps recurring groups share a link, collect sign-ups in a tap, and see who is coming — with schedules, rosters, and leaderboards built in.',
)

const webFirstPoints = [
  {
    label: 'Drop a link',
    detail: 'Works on any phone — share it in group chat and go.',
  },
  {
    label: 'No app download',
    detail: 'Guests and occasional players are in with a tap.',
  },
  {
    label: 'Three steps',
    detail: 'Open. Join. See the roster.',
  },
]
      ...
      <main className="mx-auto max-w-3xl px-5 py-12 sm:px-6 sm:py-16">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-base text-zinc-400 sm:text-lg">
            Share a link. See who&apos;s coming. Run the schedule — in the browser, no app install.
          </p>
          <ul className="mt-6 flex flex-wrap items-center justify-center gap-2">
            {['Web-based', 'No app', 'Phone sign-up', `yourgroup.${rootDomain}`].map((pill) => (
              <li
                key={pill}
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            {webFirstPoints.map(({ label, detail }) => (
              ...
              >
                ...
                <p className="mt-1 text-xs leading-snug text-zinc-400">{detail}</p>
              </div>
            ))}
          </div>
        </FeatureCard>
